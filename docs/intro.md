---
slug: /
sidebar_position: 0
---

# Bienvenid@

> "Espero te sirvan, son una porquería pero son algo :)"

## ¿Qué es esto?

Estas son mis notas. Y ya no hay más, solo que veo en internet y en mi vida diaria, gracias a todos los que me han contribuido algo los tqm.

Si se ve una porquería sin actualizar... Estoy trabajando en ello   

## Para quién son

Para mí we.

Para la persona que está empezando y no sabe por dónde. Para el que lleva tiempo pero le falta alguna pieza. Para el que quiere aprender, no para el que quiere aparentar. Para mí.

Si eres de los que se burlan de los que van empezando o de quien lo intenta, chingas a toda tu reputísima madre :).

## Cómo usarlas

Navega por el sidebar. Usa la búsqueda. No hay orden correcto. Estas notas no reemplazan la práctica. Lee, entiende, rómpelo tú mismo.

// D1ie3z — just a random one who loves hacking stuff
